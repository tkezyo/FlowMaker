...
namespace Test1.ViewModels
{
    public class FlowMakerCustomPageViewModel : ViewModelBase
    {
        ...
        public FlowMakerCustomPageViewModel(IMapper mapper, IServiceProvider serviceProvider, IMessageBoxManager messageBoxManager, IOptions<FlowMakerOption> options, FlowManager flowManager)
        {
        ...
        public async Task LoadTabs(string section)
        {
            Tabs.Clear();
            Section = section;
            var path = Path.Combine(AppContext.BaseDirectory, "CustomPage");
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }
            path = Path.Combine(path, section + ".json");
            if (!File.Exists(path))
            {
                return;
            }
